#!/usr/bin/env python3
"""Auto-generate InTouch training lessons v2 — interleaved text+images."""
import json
import os
import re
import glob
...
function saveProgress() {
  ...
  try { localStorage.setItem('intouch-lesson-progress', JSON.stringify(checks)); } catch(e) {}
  updateProgressText();
}
function loadProgress() {
  var data = null;
  try { data = localStorage.getItem('intouch-lesson-progress'); } catch(e) {}
  if (!data) { updateProgressText(); return; }
  try {
    var checks = JSON.parse(data);
    document.querySelectorAll('input[data-lesson]').forEach(function(cb) {
      if (checks[cb.dataset.lesson]) cb.checked = true;
    });
  } catch(e) {}
  updateProgressText();
}
...
def build_page_index(complete_md_path):
    ...
    for idx, pg in enumerate(sorted_pgs):
        ...
        end = page_starts[sorted_pgs[idx + 1]] if idx + 1 < len(sorted_pgs) else len(lines)
        page_index[pg] = (start, end)
    return page_index, lines


def extract_page_text(lines, start, end):
    """Extract clean text from a page range, skipping VISION blocks."""
    result = []
    in_vision = False
...
def text_to_html(text):
    """Convert extracted text to HTML paragraphs, cleaning up artifacts."""
    paragraphs = []
    current = []
    for line in text.split('\n'):
        ...
        if stripped == '':
            ...
            # Skip redundant module/section headers
            if re.match(r'^(Module \d|Section \d|Lab \d)', stripped) and len(stripped) < 80:
                continue
...
def get_vision_descriptions(complete_md_path):
    """Extract ALL VISION descriptions keyed by filename."""
    with open(complete_md_path) as f:
        content = f.read()
    visions = {}
    ...
    for m in vision_pattern.finditer(content):
        ...
        filename = f"page_{pg_num}_{sub_idx}.{ext}" if sub_idx else f"page_{pg_num}.{ext}"
...
def get_images_per_page(images_dir, page_start, page_end):
    """Get images grouped by page number."""
    all_images = sorted(glob.glob(os.path.join(images_dir, "page_*.*")))
    pages = {}
    for img_path in all_images:
        fname = os.path.basename(img_path)
        m = re.match(r'page_(\d+)(?:_(\d+))?\.(\w+)', fname)
        if m:
            pg = int(m.group(1))
            if page_start <= pg <= page_end:
                if pg not in pages:
                    pages[pg] = []
                pages[pg].append(img_path)
    return pages


def generate_interleaved_content(lines, page_index, page_start, page_end, images_per_page, visions):
    """Generate interleaved text+images HTML for the page range."""
    parts = []
    ...
    for pg in range(page_start, page_end + 1):
        ...
        if pg in page_index:
            ...
            text = extract_page_text(lines, start, end)
        ...
        if pg in images_per_page:
            ...
            for idx, img_path in enumerate(imgs):
                ...
                # For multi-image pages, try to differentiate captions
                if len(imgs) > 1:
                    # Check if there's a specific vision for this sub-image
                    if fname in visions:
                        caption = visions[fname]
                    else:
                        # Use base caption with sub-image hint
...
def main():
    ...
    print(f"Generating {len(all_lessons)} lessons (v2 interleaved)...")
    ...
    for lesson in all_lessons:
        ...
        print(f"  {lesson['id']}: {lesson['title']} ({total_imgs} images, {len(images_per_page)} pages)")

    print(f"\nDone! {len(all_lessons)} lessons generated (v2 interleaved).")
